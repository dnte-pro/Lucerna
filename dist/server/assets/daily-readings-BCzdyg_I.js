...
const CACHE_MAX_AGE = 1e3 * 60 * 60 * 24 * 8;
const POOL = [
  {
    firstReading: {
      ref: "Isaiah 55:1–3",
...
function useDailyReading(date = /* @__PURE__ */ new Date()) {
  ...
  useEffect(() => {
    ...
    fetchDailyReading(stableDate).then((liveReading) => {
      if (!alive) return;
...
async function fetchDailyReading(date = /* @__PURE__ */ new Date()) {
...
async function normalizeApiReading(data, date) {
  const season = getCurrentSeason(date);
  ...
  const [firstReadingText, psalmText, gospelText, secondReadingText] = await Promise.all([
    ...
    secondReading ? fetchBibleText(secondReading) : Promise.resolve(void 0)
  ]);
  const reading = {
    ...
    source: "api",
    notice: "Mass reading references loaded from Catholic Readings API. Verse text shown from the public-domain Douay-Rheims translation."
  };
  if (data.usccbLink) {
...
async function fetchBibleText(reference, fallback) {
...
function normalizeBibleReference(reference) {
  return reference.replace(/[–—]/g, "-").replace(/\bPsalms\b/gi, "Psalm").replace(/\bSirach\b/gi, "Ecclesiasticus").replace(/\bRevelation\b/gi, "Apocalypse").replace(/(\d+)([abc])\b/gi, "$1").replace(/\s+/g, " ").trim();
}
function formatBibleText(data) {
  if (data.verses?.length) {
    return data.verses.map((verse) => `${verse.verse}. ${verse.text.trim()}`).join("\n\n");
  }
